...
int
miner_loop(void *(*worker)(void *), int with_socket, int timer_v)
{
// blocking SIGPIPE
    sigset_t signal_set;
    ...
    if (with_socket) {
        ...
        SHA256_Transform(&sha256_ctx, (void *)&master_template);
        memcpy(master_midstate.u32, sha256_ctx.h, sizeof(master_midstate));
    ...
    struct itimerspec tspec = {
            .it_interval = { .tv_sec = timer_v, },
            .it_value = { .tv_sec = timer_v, },
    };
    ...
    for (int i = 0; i < nprocs; ++i) {
        cfgs[i].thread_id = i;
        pthread_create(&threads[i], 0, worker, &cfgs[i]);
    }
