...
__constant__ uint32_t d_midstate[2][8] = {},
                    d_data[2][16] = {},
                    d_target[2][8] = {};
__device__ uint32_t d_winning_nonce[2] = {},
                    d_block_found[2] = {},
                    d_found_hash[2][8] = {};
...
#define SHIFT_V(v) do { \
    ...
    v[4] = v[3]; v[3] = v[2]; v[2] = v[1]; \
    v[1] = v[0]; v[0] = _tmp; \
} while(0)

static __device__ __forceinline__ void
_cuda_sha256_transform_optimized2(uint32_t state[8], const uint32_t data[16])
{
    uint32_t v[8];
    #pragma unroll
    for (int i = 0; i < 8; ++i)
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        if (i < 16) {
            ...
            w_val = SCHEDULE(w[(i - 16) & 15], w[(i - 15) & 15], w[(i - 7) & 15], w[(i - 2) & 15]);
            w[(i - 16) & 15] = w_val;
        }

        ROUND_BUF(v, K[i], w_val);
        SHIFT_V(v);
    }
...
_cuda_sha256(const uint32_t data[16], uint32_t hash[8])
{
    ...
    _cuda_sha256_transform_optimized(state, data);
    for (int i = 0; i < 8; ++i)
        hash[i] = state[i];
}

static __global__ void
_cuda_sha256d(const uint32_t data[16], uint32_t hash[8])
{
    struct {
        union {
            uint8_t h8[32];
    ...
    _cuda_sha256_transform_optimized(state0, data);
    for (int i = 0; i < 8; ++i)
...
__cuda_sha256d_cont(uint32_t state0[8], const uint32_t data[16], uint32_t hash[8])
{
...
#if CUDASHA256_NOSWAP
    _cuda_sha256_transform_optimized2(state0, data);
    #pragma unroll
    for (int i = 0; i < 8; ++i)
        r2[i] = state0[i];

    _cuda_sha256_transform_optimized2(state1, r2);
    #pragma unroll
    for (int i = 0; i < 8; ++i)
    ...
    _cuda_sha256_transform_optimized2(hash, r2);
    #pragma unroll
    for (int i = 0; i < 8; ++i)
...
static __global__ void
_cuda_sha256d_cont(uint32_t state0[8], const uint32_t data[16], uint32_t hash[8])
{
...
_cuda_sha256d_btc(uint64_t start_nonce, uint32_t buf_idx)
{
    ...
    if (d_block_found[buf_idx] || (nonce > UINT32_MAX))
        return;

    uint32_t target[8];
    #pragma unroll
    for (int i = 8; i--;)
        target[i] = d_target[buf_idx][i];

    uint32_t state[8];
    #pragma unroll
    for (int i = 8; i--;)
        state[i] = d_midstate[buf_idx][i];

    uint32_t ch2_local[16];
    #pragma unroll
    for (int i = 0; i < 16; ++i)
        ch2_local[i] = d_data[buf_idx][i];
    ch2_local[3] = (uint32_t)nonce;
...
cuda_init()
{
    ...
    for (int i = 0; i < 2; i++) {
        cudaMemcpyToSymbol(d_winning_nonce[i], &z, sizeof(z));
        cudaMemcpyToSymbol(d_block_found[i], &z, sizeof(z));
...
void
cuda_sha256d(const uint32_t data[16], uint32_t hash[8])
{
    uint32_t *cuda_data;
    uint32_t *cuda_hash;
...
void
cuda_sha256d_cont(const uint32_t state[8], const uint32_t data[16], uint32_t hash[8])
{
    uint32_t *cuda_data;
    uint32_t *cuda_hash;
    uint32_t *cuda_state;
        ...
        uint32_t hash[8])
{
    ...
    if (*job_upd) {
        cudaMemcpyToSymbolAsync(d_midstate, state, 32, buf_idx? 32: 0, cudaMemcpyHostToDevice, stream);
        cudaMemcpyToSymbolAsync(d_data, data, 64, buf_idx? 64: 0, cudaMemcpyHostToDevice, stream);
        cudaMemcpyToSymbolAsync(d_target, target, 32, buf_idx? 32: 0, cudaMemcpyHostToDevice, stream);
        *job_upd -= 1;
    }
    // cudaMemcpyAsync(d_target[buf_idx], target, 32, cudaMemcpyHostToDevice, stream);
    uint32_t z = 0;
    cudaMemcpyToSymbolAsync(d_block_found, &z, 4, buf_idx? 4: 0, cudaMemcpyHostToDevice, stream);
    ...
    // _cuda_sha256d_btc<<<1, 1, 0, stream>>>(
            start_nonce,
            buf_idx
    );

    cudaMemcpyFromSymbolAsync(block_found, d_block_found, 4, buf_idx? 4: 0, cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    if (*block_found) {
        cudaMemcpyFromSymbol(winning_nonce, d_winning_nonce, 4, buf_idx? 4: 0);
        cudaMemcpyFromSymbol(hash, d_found_hash, 32, buf_idx? 32: 0);
    }
}
